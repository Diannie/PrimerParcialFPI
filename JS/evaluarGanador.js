...
function MejoresCombinacionesJugadores(tableCarts, players) {//devolver en un arreglo todos jugadores con sus mejores combionaciones
  ...
  for (var j = 0; j < players.length; j++) {
    var mejor = MejorCombinacion(allCombinaciones[j]);
    MejoresCombinaciones.push(mejor);
  }
...
function GenerarTodasCombinaciones(cartasEnMesa , jugadores) {
  var combinacionesJugadores = [];
  var limite = jugadores.length;
  for (var i = 0; i < limite; i++) {
    combinacionesJugadores.push(CombinacionesDelJugador(cartasEnMesa, jugadores[i]));
  }
